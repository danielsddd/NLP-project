"""
Training loop for Joint CRF model.

Reads the same JSONL format produced by prepare_data.py / prepare_data_merged.py:
...
from .joint_model import JointModelWithCRF
...
class JointSlotDataset(Dataset):
    """
    Loads preprocessed JSONL produced by prepare_data.py.
    ...
    def __getitem__(self, idx):
        ...
        has_mod = 1 if ex.get("has_modification", False) else 0
        ...
        return {
            ...
            "labels_slot": torch.tensor(labels, dtype=torch.long),
            "labels_intent": torch.tensor(has_mod, dtype=torch.long),
        }


# =============================================================================
# LABEL-SMOOTHED CROSS-ENTROPY (for noisy silver labels)
# =============================================================================

class LabelSmoothCE(nn.Module):
    """Cross-entropy with label smoothing for intent classification."""
    ...
    def forward(self, logits, targets):
        ...
        nll = F.nll_loss(log_p, targets, reduction="mean")
        smooth = -log_p.mean(dim=-1).mean()
        return (1 - self.eps) * nll + self.eps * smooth


import torch.nn as nn  # import here to avoid circular with LabelSmoothCE
...
def evaluate(model, loader, device):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            gold_intent = batch["labels_intent"].to(device)
            gold_slot = batch["labels_slot"]

            intent_logits, slot_preds = model(ids, mask, training=False)

            # Intent accuracy
            pred_intent = torch.argmax(intent_logits, dim=-1)
            intent_correct += (pred_intent == gold_intent).sum().item()
            intent_total += gold_intent.size(0)

            # Slot: convert to BIO label strings
            for b in range(ids.size(0)):
                ...
                for t in range(seq_len):
                    g = gold_slot[b, t].item()
                    if g == -100:
                        continue
                    gold_seq.append(BIO_ID2LABEL.get(g, "O"))
                    p = slot_preds[b][t] if t < len(slot_preds[b]) else 0
...
def train(args):
    ...
    train_ds = JointSlotDataset(train_path, max_len=args.max_len)
    val_ds = JointSlotDataset(val_path, max_len=args.max_len)
    ...
    intent_loss_fn = LabelSmoothCE(eps=0.05)
        ...
        try:
            ...
            wandb.init(
                ...
                config={
                    ...
                    "seed": args.seed,
                    "intent_loss_weight": args.intent_weight,
                },
            )
    ...
    print(f"Intent loss weight: {args.intent_weight}")
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ids = batch["input_ids"].to(device)
            mask = batch["attention_mask"].to(device)
            intent = batch["labels_intent"].to(device)
            slot = batch["labels_slot"].to(device)

            # Forward
            intent_logits, slot_loss = model(
                ids, mask,
                labels_intent=intent,
                labels_slot=slot,
                epoch=epoch,
                training=True,
            )
            ...
            # Combined loss
            loss = args.intent_weight * intent_loss + 1.0 * slot_loss
        ...
        avg_intent = epoch_intent_loss / max(n_batches, 1)
        avg_slot = epoch_slot_loss / max(n_batches, 1)
        ...
        print(f"Epoch {epoch+1}/{args.epochs}  "
              f"({elapsed:.0f}s)  "
              f"Loss={avg_loss:.4f} (intent={avg_intent:.4f} slot={avg_slot:.4f})  "
        ...
        if use_wandb:
            ...
            wandb.log({
                "epoch": epoch + 1,
                "train/loss": avg_loss,
                "train/intent_loss": avg_intent,
                "train/slot_loss": avg_slot,
                "train/lr": scheduler.get_last_lr()[0],
                "val/f1": metrics["f1"],
                "val/precision": metrics["precision"],
                "val/recall": metrics["recall"],
                "val/intent_accuracy": metrics["intent_accuracy"],
                "scheduled_gold_prob": model._gold_probability(epoch),
            })
    ...
    print(f"  Intent:    {metrics['intent_accuracy']:.4f}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Joint Intent+Slot CRF model"
    )
    # Model
    ...
    parser.add_argument("--lr", type=float, default=2e-5,
                        help="Learning rate for BERT layers (heads get 5x)")
    parser.add_argument("--intent-weight", type=float, default=0.3,
                        help="Weight for intent loss (slot loss weight = 1.0)")
