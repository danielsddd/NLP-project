...
def get_spans_from_io(seq):
    """Extract (start, end, type) for IO tag sequences (no B- tags)."""
    spans = []
    i = 0
    while i < len(seq):
        tag = seq[i]
        if tag != "O":
            start = i
            etype = tag[2:] if tag.startswith("I-") else tag
            i += 1
            while i < len(seq) and seq[i] == tag:
                i += 1
...
def compute_relaxed_metrics(true_seqs, pred_seqs):
    ...
    return {"relaxed_precision": round(p, 4),
...
def compute_token_level_f1(true_seqs, pred_seqs):
    ...
    return {"token_precision": round(p, 4),
...
def evaluate(model_path, test_file, output_dir="results", compute_bootstrap=False):
    ...
    print(f"  Relaxed F1:      {relaxed['relaxed_f1']:.4f}   (P={relaxed['relaxed_precision']:.4f}  R={relaxed['relaxed_recall']:.4f})")
    print(f"  Token   F1:      {token['token_f1']:.4f}   (P={token['token_precision']:.4f}  R={token['token_recall']:.4f})")
    print(f"\n  Per-Aspect F1:")
