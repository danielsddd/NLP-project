...
Fixes applied:
    - _map_norm_to_orig: fixed space-handling bug (normalize_hebrew on single chars
      strips spaces via .strip(), breaking position mapping for multi-word spans)
    - _map_norm_to_orig: removed dead variables (norm_pos, orig_pos, start_orig, norm_orig)
    - CLI: added --val-split and --test-split args (SLURM scripts pass them)
"""
...
def get_source_text(record):
    ...
    # By design, the teacher labels one source comment per thread,
    # so all mods in a record share the same source_comment.
...
def _map_norm_to_orig(original, normalized, norm_idx, norm_len):
    ...
    Builds a character-level mapping by replaying the normalization steps
    (strip punctuation → translate finals → collapse whitespace → strip)
    on the original string, tracking which original index produced each
    character of the normalized string.

    FIX: The old version called normalize_hebrew() per character. That
    function ends with .strip(), which kills standalone spaces, so the
    mapping broke for any span containing whitespace. We now replay the
    normalization logic inline without .strip() per char.
    """
    # Step 1: Build intermediate string after punctuation removal + finals,
    #         tracking original indices.
    intermediate = []  # list of (orig_index, char_after_finals)
    for oi, ch in enumerate(original):
        # Same regex as normalize_hebrew: keep only \w, \s, Hebrew
        if not re.match(r'[\w\s\u0590-\u05FF]', ch):
            continue  # punctuation stripped
        # Apply final-letter normalization
        mapped = ch.translate(_FINALS)
        intermediate.append((oi, mapped))

    # Step 2: Collapse whitespace (replicate re.sub(r'\s+', ' ', ...))
    collapsed = []  # list of (orig_index, char)
    ...
    # Step 3: Strip leading/trailing spaces (replicate .strip())
    while collapsed and collapsed[0][1] == ' ':
        collapsed.pop(0)
    while collapsed and collapsed[-1][1] == ' ':
        collapsed.pop()

    # collapsed[i] = (original_index, normalized_char) for each position i
    # in the normalized string. Look up norm_idx directly.
    if norm_idx < len(collapsed):
        return collapsed[norm_idx][0]
...
def main():
    ...
    # Split ratios — also accepted from SLURM scripts
