...
PROJECT_DIR="simanovsky2_DIR/$PROJECT_NAME"
...
mkdir -p simanovsky2_DIR/.cache/{huggingface,torch}
mkdir -p simanovsky2_DIR/logs
...
if [ -d "$HOME/.cache/huggingface" ] && [ ! -L "$HOME/.cache/huggingface" ]; then
    ...
    mv $HOME/.cache/huggingface/* simanovsky2_DIR/.cache/huggingface/ 2>/dev/null
...
if [ ! -L "$HOME/.cache/huggingface" ]; then
    mkdir -p $HOME/.cache
    ln -sf simanovsky2_DIR/.cache/huggingface $HOME/.cache/huggingface
    echo "  Created symlink: ~/.cache/huggingface -> simanovsky2_DIR/.cache/huggingface"
else
...
if [ ! -L "$HOME/.cache/torch" ]; then
    ...
    ln -sf simanovsky2_DIR/.cache/torch $HOME/.cache/torch
    echo "  Created symlink: ~/.cache/torch -> simanovsky2_DIR/.cache/torch"
fi
...
if ! command -v conda &> /dev/null; then
    ...
    bash Miniconda3-latest-Linux-x86_64.sh -b -p simanovsky2_DIR/anaconda3
    rm Miniconda3-latest-Linux-x86_64.sh
    
    # Add to path
    export PATH="simanovsky2_DIR/anaconda3/bin:$PATH"
    echo "  Miniconda installed"
fi

# Initialize conda
source simanovsky2_DIR/anaconda3/etc/profile.d/conda.sh 2>/dev/null || source ~/anaconda3/etc/profile.d/conda.sh 2>/dev/null
...
USER_DIR=/vol/joberant_nobck/data/NLP_368307701_2526a/simanovsky2
PROJECT_DIR=simanovsky2_DIR/recipe_modification_extraction
HF_HOME=simanovsky2_DIR/.cache/huggingface
ENVFILE
...
echo "2. Upload your code: rsync -avz ./src/ simanovsky2@nova:$PROJECT_DIR/src/"
